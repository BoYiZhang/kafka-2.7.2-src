...
/**
 *
 * KafkaRequestHandler 是一个 Runnable 对象，因此，你可以把它当成是一个线程。
 *
 * 请求处理线程类。每个请求处理线程实例，
 * 负责从SocketServer 的 RequestChannel 的请求队列中获取请求对象，并进行处理。
 *
 * 关键字段说明 :
 *   id: I/O线程序号
...
//  requestChannel：SocketServer 中的请求通道对象。
//      KafkaRequestHandler对象为什么要定义这个字段呢？
//      我们说过，它是负责处理请求的类，那请求保存在什么地方 呢？
//      实际上，请求恰恰是保存在 RequestChannel 中的请求队列中，因此，Kafka 在构 造 KafkaRequestHandler 实例时，
//      必须关联 SocketServer 组件中的 RequestChannel 实例，
//      也就是说，要让 I/O 线程能够找到请求被保存的地方。
//  apis：这是一个KafkaApis 类。
//        如果说KafkaRequestHandler是真正处理请求的，
//        那么，KafkaApis 类就是真正执行请求处理逻辑的地方。
...
//  requestChannel：
//            SocketServer 的请求处理通道，它下辖的请求队列为所有 I/O 线程 所共享。
//            requestChannel 字段也是 KafkaRequestHandler 类的一个重要属性。
//  apis：KafkaApis 实例，执行实际的请求处理逻辑。
//            它同时也是 KafkaRequestHandler 类的一个重要属性。
//  numThreads：线程池中的初始线程数量。它是 Broker 端参数 num.io.threads 的值。
//              目前，Kafka 支持动态修改 I/O 线程池的大小，因此，这里的 numThreads 是初始线程 数，
...
class KafkaRequestHandlerPool(val brokerId: Int,
  ...
  // I/O线程池
  // 控制 KafkaRequestHandler 的数量
  val runnables = new mutable.ArrayBuffer[KafkaRequestHandler](numThreads)
  for (i <- 0 until numThreads) {
    createHandler(i)
  }

  // 构建 KafkaRequestHandler
